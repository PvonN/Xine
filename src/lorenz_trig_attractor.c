...
typedef struct {
  ...
  MYFLT *c_trig, *reset_trig, *sigma, *rho, *beta, *delta_time, *skip, *in_x, *in_y, *in_z; /* input */
  MYFLT x_value, y_value, z_value, x_in, y_in, z_in; /* internal variables */
} LORENZ2T;

int32_t lorenz2t_init(CSOUND *csound, LORENZ2T *p){
  p->x_value = *p->in_x;
  p->y_value = *p->in_y;
  p->z_value = *p->in_z;
...
int32_t lorenz2t_process(CSOUND *csound, LORENZ2T *p){
  ...
  for (sample_index = offset; sample_index <
	...
	/* if c_trig is != 0 compute values and write them to audio vectors */
	if (*p->c_trig)
	  {
		/* get values loop */
		do {
	  ...
	  /* if c_trig is == 0 write 0's to audio vectors */
      x_in = 0;
      y_in = 0;
      z_in = 0;
    }
